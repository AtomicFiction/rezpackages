...
if [ -d objdir ]; then
    ...
    ../gcc-${gcc_ver}/configure --prefix=${install_dir} --enable-languages=c,c++ --with-pic --enable-shared --enable-static --enable-threads=posix --enable-__cxa_atexit --enable-clocale=gnu --with-libelf=/usr/include --disable-multilib --disable-bootstrap --disable-install-libiberty --with-system-zlib
fi
